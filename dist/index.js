...
const GLOB_MDX = "**/*.mdx";
const GLOB_MARKDOWN_OR_MDX = "**/*.{md,mdx}";
const GLOB_MARKDOWN_OR_MDX_IN_MARKDOWN_OR_MDX = "**/*.{md,mdx}/*.{md,mdx}";
...
const GLOB_MARKDOWN_CODE = `${GLOB_MARKDOWN_OR_MDX}/${GLOB_SRC}`;
const GLOB_TESTS = [
...
async function formatters(options = {}, stylistic$1 = {}, markdownParserEnabled = true) {
	if (options === true) {
...
async function markdown(options = {}) {
	...
	const patchedParser = {
		...mdxParser,
		parse(text, options$1) {
			const result = mdxParser.parseForESLint(text, options$1);
			const body = result.ast.body;
			function predicate(token) {
				for (const node$1 of body) if (node$1.start <= token.start && node$1.end >= token.end) return true;
				return false;
			}
	...
	return [{
		...
		files: [GLOB_MARKDOWN_CODE, ...componentExts.map((ext) => `${GLOB_MARKDOWN_OR_MDX}/**/*.${ext}`)],
...
async function typescript(options = {}) {
	...
	return [
		...
		...erasableOnly ? [{
			...
			plugins: { "erasable-syntax-only": await interopDefault(import("./lib-Cm-GRsZW.js")) },
			rules: {
...
function antfu(options = {}, ...userConfigs) {
	...
	if (options.formatters) configs$1.push(formatters(options.formatters, typeof stylisticOptions === "boolean" ? {} : stylisticOptions, options.markdown !== false));
